...
if [[ ! -d "$COMMANDS_DIR" ]]; then
    echo "ERROR: $COMMANDS_DIR が見つからない〜。vault のルートで実行してる？" >&2
    exit 1
fi
...
if [[ -n "$PARTNER_NAME" ]]; then
    ...
    "${SED_INPLACE[@]}" \
        ...
        -e '/{{PARTNER_NAME}}/d' \
        "$COMMANDS_DIR"/*.md
    echo "  共有カレンダーは使わない設定にしたよ〜（該当行を削除）"
fi
...
if [[ -n "$CALENDAR_ID_PRIVATE" ]]; then
    "${SED_INPLACE[@]}" \
        ...
        "$COMMANDS_DIR"/*.md
    echo "  プライベートカレンダーのプレースホルダーを置換したよ〜"
else
    "${SED_INPLACE[@]}" \
        -e '/{{CALENDAR_ID_PRIVATE}}/d' \
        "$COMMANDS_DIR"/*.md
    echo "  プライベートカレンダーは使わない設定にしたよ〜（該当行を削除）"
fi
...
if [[ -f "$CONFIG_FILE" ]]; then
    echo ""
    echo "  $CONFIG_FILE は既に存在してるからスキップ〜"
else
